#!/usr/bin/env node
/**
 * 拉取劲牌三产品 7/23 的情感分析 + 分平台词条明细
 * 输出: src/data/july23_entries_sentiments.json
 */
...
async function main() {
  ...
  const outPath = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../src/data/july23_entries_sentiments.json');
