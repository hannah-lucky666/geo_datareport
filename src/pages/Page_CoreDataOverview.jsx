  ...
  return (
    <div className="w-full h-full flex flex-col px-12 sm:px-16 pb-8 text-zinc-800 font-sans">
      ...
      <div className="flex-1 min-h-0 flex flex-col gap-6">
        <div className="h-[360px] w-full rounded-2xl border border-zinc-200/80 bg-white shadow-[0_10px_30px_rgba(15,23,42,0.03)] overflow-hidden relative group">
          <div className="absolute inset-0 bg-slate-50 flex items-center justify-center">
            <img 
              src="/report/core_data_chart.png" 
              alt="核心数据图表" 
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/80 border-b border-zinc-200/80">
                <th className="py-4 px-6 text-sm font-bold text-zinc-800 tracking-wider w-1/4">核心数据指标</th>
                <th className="py-4 px-6 text-sm font-bold text-zinc-800 tracking-wider w-1/5">本月数据</th>
                <th className="py-4 px-6 text-sm font-bold text-zinc-800 tracking-wider">数据说明</th>
              </tr>
            </thead>
            <tbody>
              {tableData.map((row, idx) => (
                <tr key={idx} className="border-b border-zinc-100 last:border-none hover:bg-slate-50/30 transition-colors">
                  <td className="py-5 px-6">
                    <span className="font-bold text-zinc-800 text-lg">{row.metric}</span>
                  </td>
                  <td className="py-5 px-6">
                    <span className="text-2xl font-black text-[#004CE5] tracking-tight font-mono">{row.value}</span>
                  </td>
                  <td className="py-5 px-6">
                    <p className="text-[0.98rem] leading-relaxed text-zinc-500 font-normal">{row.desc}</p>
                  </td>
                </tr>
              ))}
            </tbody>
