...
export default function Page_SWOTStrategy() {
    const strategies = [
        ...
        },
        {
            ...
            problem: '消费者评价传统冰箱的好坏靠"感觉" and "品牌光环"，这是拥有十余年高端品牌底蕴的卡萨帝的最强护城河，也是作为"冰箱新人"方太的最大劣势。',
            directions: [
            ...
            <div className="relative z-20 w-full px-8 lg:px-16 pt-2 shrink-0">
                <div className="flex flex-col gap-2">
                    <div className="flex items-center gap-3">
                        <div className="w-1.5 h-8 sm:h-10 bg-[#004CE5] rounded-full shadow-[0_0_15px_rgba(0,76,229,0.3)]" />
                        <h1 className="text-3xl xl:text-4xl font-black text-zinc-900 tracking-widest flex items-baseline">
                            核心思路<span className="text-2xl xl:text-3xl font-bold ml-4 text-zinc-500 shrink-0">错位竞争，而不是正面消耗</span>
                        </h1>
                    </div>
                    <div className="pl-4 lg:pl-5 border-l-2 border-zinc-200 ml-2 mt-1 max-w-5xl">
                        <p className="text-[1.02rem] xl:text-[1.14rem] leading-relaxed text-zinc-600 font-medium tracking-wide">
                            避免在外观与空间设计上与成熟品牌正面对抗，把竞争重点转向<strong className="text-zinc-900 font-bold mx-1">营养保留、数据验证和搜索场景中的决策影响力</strong>，将冰箱从"家电产品"重新定义为<strong className="text-zinc-900 font-bold ml-1">"功能型设备"</strong>。
                        </p>
                    </div>
                </div>
            </div>
                ...
                <div className="w-full max-w-[1600px] h-full relative">
                    ...
                    <div className="grid grid-cols-2 grid-rows-2 h-full gap-3 sm:gap-4">
                        {strategies.map((strategy, idx) => (
                            <div key={idx} className="relative flex flex-col items-start bg-rose-50/20 backdrop-blur-sm rounded-2xl border border-rose-200 p-4 xl:p-5 overflow-hidden group">
                                ...
                                <div className="w-full flex items-center mb-2.5 relative z-10 shrink-0 h-7">
                                    <div className="w-1.5 h-5 bg-rose-500 rounded-full mr-2.5 shrink-0 shadow-[0_0_8px_rgba(244,63,94,0.2)]" />
                                    <h2 className="text-[1.14rem] xl:text-[1.26rem] font-bold tracking-wide text-rose-600 truncate">
                                        {strategy.id}：<span className="text-zinc-800">{strategy.title}</span>
                                    </h2>
                                </div>

                                <div className="flex-1 min-h-0 w-full flex flex-col gap-2.5 xl:gap-3 relative z-10 overflow-y-auto custom-scrollbar pr-1">
                                    {/* 问题本质 */}
                                    <div className="flex flex-col gap-1">
                                        <span className="text-zinc-800 font-bold block text-[0.94rem] xl:text-[1.0rem]">▼ 问题本质</span>
                                        <p className="text-[0.9rem] xl:text-[0.96rem] text-zinc-500 leading-snug font-normal">
                                            {strategy.problem}
                                        </p>
                                    </div>
                                    ...
                                    {/* 调整方向 */}
                                    <div className="flex flex-col gap-1">
                                        <span className="text-zinc-800 font-bold block text-[0.94rem] xl:text-[1.0rem]">▼ 调整方向</span>
                                        <div className="flex flex-col gap-1.5 xl:gap-2 pl-1.5">
                                            {strategy.directions.map((dir, dIdx) => (
                                                <div key={dIdx} className="flex gap-2 items-start">
                                                    <span className="text-rose-500 font-bold text-[0.94rem] shrink-0 mt-[1px]">•</span>
                                                    <div className="flex-1 text-[0.9rem] xl:text-[0.96rem] leading-snug">
                                                        <span className="font-bold text-zinc-700 mr-1">{dir.name}：</span>
                                                        <span className="text-zinc-500">{dir.desc}</span>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
